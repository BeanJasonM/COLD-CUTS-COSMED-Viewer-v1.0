...
def _safe_auc(x: pd.Series, y: pd.Series) -> float:
    ...
    if valid.sum() < 2:
        return np.nan
    x_values = pd.to_numeric(x.loc[valid], errors="coerce").to_numpy(dtype=float)
    y_values = pd.to_numeric(y.loc[valid], errors="coerce").to_numpy(dtype=float)
    ...
    dx = np.diff(x_values)
    average_heights = (y_values[:-1] + y_values[1:]) / 2
    return float(np.sum(dx * average_heights))
