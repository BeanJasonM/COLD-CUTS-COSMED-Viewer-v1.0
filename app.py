...
def auto_detect_epochs(df: pd.DataFrame, time_col: str) -> list[dict[str, Any]]:
    """Create simple epochs automatically from Phase or Marker columns when available."""
    ...
    if "Phase" in df.columns:
        epochs = []
        phase_series = df["Phase"].astype(str).replace({"nan": ""}).str.strip()
        phase_block = (phase_series != phase_series.shift()).cumsum()
        for _, subset in df.groupby(phase_block):
            label = str(subset["Phase"].iloc[0]).strip()
            if not label or label.lower() == "nan":
                continue
            start = float(time_values.loc[subset.index].min())
            end = float(time_values.loc[subset.index].max())
            epochs.append({"epoch": label, "start": start, "end": end})
        if epochs:
            return epochs

    if "Marker" in df.columns:
        marker_rows = df[df["Marker"].astype(str).str.strip().ne("")]
        markers = []
        for idx, row in marker_rows.iterrows():
            markers.append({
                "label": str(row["Marker"]).strip(),
                "time": seconds_to_label(time_values.loc[idx]),
                "seconds": float(time_values.loc[idx]),
            })
        marker_df = pd.DataFrame(markers)
        if not marker_df.empty:
            return create_epochs_from_markers(marker_df)
...
def overview_cards(summary: pd.DataFrame) -> None:
    """Render compact visual cards for selected-variable summaries."""
    if summary.empty or "Variable" not in summary.columns:
        return
    st.subheader("Researcher Snapshot")
    columns = st.columns(min(4, len(summary)))
    for index, (_, row) in enumerate(summary.head(8).iterrows()):
        col = columns[index % len(columns)]
        mean_value = row.get("Mean", np.nan)
        change_value = row.get("Percent change", np.nan)
        peak_value = row.get("Peak value", np.nan)
        col.markdown(
            f"""
            <div class="status-card">
              <div class="status-label">{row.get("Variable", "Variable")}</div>
              <div class="status-value">Mean: {mean_value:.3g}</div>
              <div class="muted">Peak: {peak_value:.3g} | Change: {"N/A" if pd.isna(change_value) else f"{change_value:.1f}%"}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def missingness_heatmap(missing_summary: pd.DataFrame) -> go.Figure:
    """Make a compact missingness heatmap."""
    if missing_summary.empty:
        return go.Figure()
    fig = go.Figure(data=go.Heatmap(
        z=[missing_summary["Missing %"].tolist()],
        x=missing_summary["Variable"].tolist(),
        y=["Missing %"],
        colorscale=[[0, "#E8F5E9"], [0.5, "#ED9B40"], [1, "#C62828"]],
        zmin=0,
        zmax=100,
        colorbar={"title": "%"},
    ))
    fig.update_layout(height=230, plot_bgcolor="#FFFFFF", paper_bgcolor="#FFFFFF", font={"color": "#17212B"})
    return fig
...
epochs = auto_detect_epochs(processed_df, time_col)
marker_table = pd.DataFrame()
with st.expander("Advanced add-ons: custom phases and epoch tools", expanded=False):
    st.caption("Optional. The app already analyzes the full selected time range. Use this only when you want custom phase summaries.")
    epoch_method = st.radio(
        "Phase method",
        ["Use automatic phases", "Manual start/end markers", "Event marker builder", "Import epoch file"],
        horizontal=True,
    )
    if epoch_method == "Use automatic phases":
        st.write("Automatic phases are built from `Phase` first, then `Marker` if available.")
    elif epoch_method == "Manual start/end markers":
        ...
        imported_epochs = load_epoch_file(uploaded_epoch_file)
        if imported_epochs:
...
with tabs[0]:
    ...
    overview_cards(variable_summary)
    st.subheader("Variable Summary")
    st.dataframe(variable_summary, width="stretch")
    if not variable_summary.empty and "Mean" in variable_summary.columns:
        mean_fig = px.bar(
            variable_summary.dropna(subset=["Mean"]),
            x="Variable",
            y="Mean",
            color="Variable",
            title="Mean by Selected Variable",
            color_discrete_sequence=PLOT_COLORS,
        )
        mean_fig.update_layout(plot_bgcolor="#FFFFFF", paper_bgcolor="#FFFFFF", font={"color": "#17212B"}, showlegend=False)
...
with tabs[4]:
    ...
    with qc_cols[0]:
        st.write("QC flags by category")
        st.dataframe(qc_summary, width="stretch")
        if not qc_summary.empty:
            qc_fig = px.bar(qc_summary, x="Issue", y="Flags", color="Issue", color_discrete_sequence=PLOT_COLORS)
            qc_fig.update_layout(plot_bgcolor="#FFFFFF", paper_bgcolor="#FFFFFF", font={"color": "#17212B"}, showlegend=False)
            st.plotly_chart(qc_fig)
    with qc_cols[1]:
        st.write("Missingness by selected variable")
        st.dataframe(missing_summary, width="stretch")
        st.plotly_chart(missingness_heatmap(missing_summary))
    if qc_report.empty:
